...
export const Slider: (
  props: SliderProperties & RefAttributes<ComponentInternals>
  ...
  (
    ...
    const handler = useMemo(() => {
      ...
      function setValue(e: ThreeEvent<PointerEvent>) {
        ...
        const newValue = Math.min(
          Math.max(
            Math.round(((vectorHelper.x + 0.5) * (max - min) + min) / step) *
              step,
            min
          ),
          max
    ...
    const range = max - min;
    const width =
      `${((1 - 0.03) * clamp(value / range, 0, 1) + 0.03) * 100}%` as const;
        ...
        <Container positionType="absolute" inset={-2}>
          ...
          >
            ...
            <Container
              ...
              backgroundColor={colors.foreground}
              backgroundOpacity={
                disabled ? 0 : 1 /** todo use hovered from root element */
              }
            />
          </Container>
        </Container>
