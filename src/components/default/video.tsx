...
export const Video: (
  props: VideoProperties & RefAttributes<VideoInternals>
...
export const VideoControls: (
  ...
  if (moving == null) {
    throw new Error(
      `VideoControls form the default kit can only be used inside a Video from the default kit`
    );
  }
  const displaySignal = useMemo(
    () => computed(() => (moving.value ? "flex" : "none")),
    [moving]
  );

  useEffect(() => {
    const metadataListener = () =>
      (durationSignal.value = videoElement.duration);
    const timeUpdateListener = () =>
      (timeSignal.value = videoElement.currentTime);
  ...
  const setTime = useCallback(
    (t: number) => (videoElement.currentTime = t),
    [videoElement]
  );
    ...
    >
      <Container flexDirection="row" alignItems="center">
        ...
        <Button
          ...
          variant="ghost"
          marginRight={8}
          onClick={() => (videoElement.muted = !muted)}
        >
          {muted ? (
